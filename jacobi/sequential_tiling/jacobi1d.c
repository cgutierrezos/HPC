...
int BLOCK_SIZE = 10240; // Definir según la caché
...
void jacobi(int nsweeps, int n, double* u, double* f) {
    ...
    for (int sweep = 0; sweep < nsweeps; sweep++) {
        for (int block_start = 1; block_start < n; block_start += BLOCK_SIZE) {
            int block_end = block_start + BLOCK_SIZE < n ? block_start + BLOCK_SIZE : n;
            for (int i = block_start; i < block_end; i++) {
                // Cálculo estándar de Jacobi aquí, asegurándose de manejar correctamente los bordes del bloque
